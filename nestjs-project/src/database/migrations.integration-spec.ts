...
import { CreateVideos1782673690213 } from './migrations/1782673690213-CreateVideos';
...
describe('Database migrations (integration)', () => {
  ...
  it('should revert the last migration and remove the videos table + enum', async () => {
    ...
    const tablesResult = await dataSource.query<{ table_name: string }[]>(
      `SELECT table_name FROM information_schema.tables
       WHERE table_schema = 'public'
    ...
    const typesResult = await dataSource.query<{ typname: string }[]>(
