...
type RegisterBody = { id: string; email: string };
type TokensBody = { access_token: string; refresh_token: string };
type MeBody = { sub: string; email: string };
...
describe('Auth (e2e)', () => {
  ...
  async function captureConfirmationToken(
    ...
    const mailService = Reflect.get(app.get(AuthService), 'mailService') as {
  ...
  async function registerConfirmAndLogin(
    ...
    const res = await request(app.getHttpServer())
      .post('/auth/login')
      .send({ email, password });
    const tokens = bodyOf<TokensBody>(res);
  ...
  describe('POST /auth/register', () => {
    it('returns 201 with { id, email } on valid registration', async () => {
      const res = await request(app.getHttpServer())
        ...
        .expect(201);
      const body = bodyOf<RegisterBody>(res);
    ...
    it('returns 409 with EMAIL_ALREADY_EXISTS on duplicate email', async () => {
      ...
      expect(bodyOf<ErrorBody>(res).error).toBe('EMAIL_ALREADY_EXISTS');
    });
  ...
  describe('GET /auth/confirm-email', () => {
    ...
    it('returns 401 with INVALID_TOKEN on an already-used token', async () => {
      ...
      expect(bodyOf<ErrorBody>(res).error).toBe('INVALID_TOKEN');
    });
    ...
    it('returns 401 with TOKEN_EXPIRED on an expired token', async () => {
      ...
      expect(bodyOf<ErrorBody>(res).error).toBe('TOKEN_EXPIRED');
    });
  ...
  describe('POST /auth/login', () => {
    ...
    it('returns 200 with access_token and refresh_token on valid credentials', async () => {
      ...
      const res = await request(app.getHttpServer())
        ...
        .expect(200);
      const body = bodyOf<TokensBody>(res);
    ...
    it('returns 401 with INVALID_CREDENTIALS on wrong password', async () => {
      ...
      expect(bodyOf<ErrorBody>(res).error).toBe('INVALID_CREDENTIALS');
    });
    ...
    it('returns 403 with EMAIL_NOT_CONFIRMED when user is not confirmed', async () => {
      ...
      expect(bodyOf<ErrorBody>(res).error).toBe('EMAIL_NOT_CONFIRMED');
    });
  ...
  describe('POST /auth/refresh', () => {
    ...
    it('returns 200 with valid access token when reuse is within grace period', async () => {
      ...
      expect(bodyOf<TokensBody>(res).access_token).toBeDefined();
    ...
    it('returns 401 with TOKEN_REUSE_DETECTED when reuse is beyond grace period', async () => {
      ...
      expect(bodyOf<ErrorBody>(res).error).toBe('TOKEN_REUSE_DETECTED');
  ...
  describe('POST /auth/reset-password', () => {
    it('returns 204 with a valid token and updates the password', async () => {
      ...
      const loginRes = await request(app.getHttpServer())
        ...
        .expect(200);
      expect(bodyOf<TokensBody>(loginRes).access_token).toBeDefined();
    });
