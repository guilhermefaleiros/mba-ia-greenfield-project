...
- **Observations:**
  ...
  - The `videos.module.spec.ts` compilation test needs the same config graph as `VideosModule -> AuthModule -> MailModule`; loading `appConfig`, `authConfig`, `mailConfig`, `storageConfig`, and `queueConfig` in the test module closes the DI graph and keeps the module-compilation check green.
  - The queue-backed compile path still opens a real Redis connection during `QueueModule` bootstrap, but the suite no longer hangs once the dependent config graph is loaded correctly.
...
- **Observations:**
  ...
  - The auth e2e bootstrap no longer regresses on BullMQ: `test/auth.e2e-spec.ts` overrides `VideosModule` with a test stub so the suite keeps exercising auth via `AppModule` without pulling the video queue/Redis path into compile-time setup.
